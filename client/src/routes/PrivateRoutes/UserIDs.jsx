...
async function getUser() {
    try {
        const res = await fetch (`https://onestmern-ecommerce-12.onrender.com/get-User/${params.id}`)
...
const getOrders = async ()=>{
    try {
    const res =  await fetch(`https://onestmern-ecommerce-12.onrender.com/orders/${params.id}` , {
    })
    const data = await res.json()
                    ...
                    {o?.products.map((p,i) => (
                        <div className="row mb-2 card flex-row" key={p._id}>
                            <div className="col-md-4">
                                <img 
                                    src={`https://onestmern-ecommerce-12.onrender.com/product-photo/${p._id}`}
