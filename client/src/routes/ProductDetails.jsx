...
const getProduct = async () => {
    try {
        const res = await fetch (`https://mern-ecomm-62pn.onrender.com/get-product/${params.slug}`)
...
const getSimilarProduct =async (pid,cid)=>{
  try {
    const res = await fetch(`https://mern-ecomm-62pn.onrender.com/related-product/${pid}/${cid}`)
 const data = await res.json()
...
<div className="col-md-5 mb-2 "   > 
    <img  src={`https://mern-ecomm-62pn.onrender.com/product-photo/${product._id}`}      style={{height:"27rem" , width:"32rem " , marginLeft:"5rem"  }}   className="card" alt={product.name} />
...
{relatedProducts.map((p)=>  <div  className='card'  style={{width:"17rem" , margin:'0.5rem'  }}  key={p._id}     >  
    ...
    <img    src={`https://mern-ecomm-62pn.onrender.com/product-photo/${p._id}`}     className='card-img-top'  style={{height:"16rem" , borderBottom:"0.2px solid grey  " }}  alt={p.name} />
