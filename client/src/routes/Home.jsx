...
const getTotal = async ()=>{
  try {
    ...
    const res = await fetch("https://onestmern-ecommerce-10.onrender.com/product-count")
...
const getAllProducts = async ()=>{
  try {
    setLoading(true)
    const res =  await fetch (`https://onestmern-ecommerce-10.onrender.com/product-list/${page}`)
    const data = await res.json()    
...
const filterProducts = async ()=>{
  try {
    const res = await fetch(`https://onestmern-ecommerce-10.onrender.com/product-filters`, {
      method : "POST",
      headers:{
...
{product.map((p)=>  <div  className='card'  style={{width:"17rem" , margin:'0.5rem'  }}  key={p._id} >  
    <img src={`https://onestmern-ecommerce-10.onrender.com/product-photo/${p._id}`}     className='card-img-top'  style={{maxHeight:"16rem" , borderBottom:"0.2px solid grey  " }}  alt={p.name} />
